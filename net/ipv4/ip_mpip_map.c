...
int update_path_delay(unsigned char path_id, __s32 delay)
{
	...
	list_for_each_entry(path_info, &pi_head, list)
	{
		...
		{
			path_info->delay = (3 * path_info->delay + delay) / 4;
...
int update_path_info()
{
	...
	list_for_each_entry(path_info, &pi_head, list)
	{
		...
		{
			path_info->bw += path_info->max_delay_diff * 
					 path_info->max_delay_diff * 
					 sysctl_mpip_bw_factor;
		}
		else
		{

			path_info->bw +=
				(path_info->max_delay_diff * path_info->max_delay_diff) / 
				(path_info->delay_diff * path_info->delay_diff) * 
				sysctl_mpip_bw_factor;
		}
